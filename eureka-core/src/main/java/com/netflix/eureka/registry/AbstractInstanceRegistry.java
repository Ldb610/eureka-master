...
public abstract class AbstractInstanceRegistry implements InstanceRegistry {
     ...
     *
     * @param registrant 注册信息
     * @param leaseDuration 表示eureka server至上一次收到client的心跳之后，等待下一次心跳的超时时间，在这个时间内若没收到下一次心跳，则将移除该instance
     * @param isReplication 这是否是来自另一个eureka节点的复制条目。
     */
    public void register(InstanceInfo registrant, int leaseDuration, boolean isReplication) {
        try {
            ...
            read.lock();
            /**
             * Map<String, Lease<InstanceInfo>> ：String-> appName 客户端名称；Lease<InstanceInfo> 通过微服务名拿到的微服务组
             * Lease：租债器对象
             */
            ...
            if (gMap == null) {
                ...
                // 如果微服务组 是空的，那么就新建一个 ConcurrentHashMap 容器来存放新的微服务组的信息
            ...
            // 根据微服务的注册id 去获取已经存在的注册信息
            Lease<InstanceInfo> existingLease = gMap.get(registrant.getId());
            // Retain the last dirty timestamp without overwriting it, if there is already a lease
            // 保留最后一个脏时间戳，而不覆盖它(如果已经有租约)
            /**
             * 进去这个if 的情况不多，需要已经存在相同的 id的eureka-client 注册了，正常来说是不太可能发生这样的情况，打断点的时候会发生
             * 还有就是两个客户端的id命名成一样的了，其它情况还没遇到
             *
             * existingLease.getHolder() 获取具体的客户端实例
             */
            if (existingLease != null && (existingLease.getHolder() != null)) {

                // 从租债器获取已经存在微服务的最后的操作时间
            ...
            if (existingLease != null) {
                // 设置服务正常的时间，例如20:00:00第一次来注册，服务正常时间时20:00:00 ,然后eurekaService挂了，然后20:20:20又重新注册成功，那就会变成 20:20:20
                // 如果客服端挂了，再重新连接，这个值不变
    ...
    /**
     * 自我保护机制阈值计算:    预估心跳值(所有注册上来的实例) * 每分钟触发的心跳连接次数( 60s/服务端每分钟心跳连接刷新时间(默认30s) * 自我保护机制的触发百分比85%)
     */
